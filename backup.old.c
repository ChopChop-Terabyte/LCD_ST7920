...
// uint8_t Clean[1024] = {[0 ... 1023] = 0x00};

// uint8_t Ready_change = 1;
uint8_t Ready_change = 0;
...
static void LCD_I2C_init(i2c_master_bus_handle_t *bus_handle, i2c_master_dev_handle_t *dev_handle) {
...
void LCD_Rotate_bitmap(uint8_t *bitmap, uint8_t *rotate) {
...
void LCD_draw_screen(uint8_t *bitmap) {
    int count = 0, index = 0;
...
void Screen_1(uint8_t *screen) {
    while (Ready_change) vTaskDelay(1 / portTICK_PERIOD_MS);
    // while (Ready_change);
    Ready_change = 1;
    ...
    LCD_Graphic_send_text(screen, "    U(V)        XXX.XX       XXX.XX       XXX.XX", 0, 27);
    LCD_Graphic_send_text(screen, "    I(A)        XXX.XX       XXX.XX       XXX.XX", 0, 36);
    LCD_Graphic_send_text(screen, "  P(kW)      XXX.XX       XXX.XX       XXX.XX", 0, 45);
    LCD_Graphic_send_text(screen, "  X.XXXkW    XXXXXXkWh    Cos: X.XXX", 0, 55);
...
void Network_signal(uint8_t *screen, uint8_t num) {        //num: 1, 2 = Low | 3 = Medium | 4, 5 = High | 0 = No signal
    switch (num) {
        case 1:
            LCD_handle_text(screen, 0, 0, signal_1);
            break;
        case 2:
            LCD_handle_text(screen, 0, 0, signal_1);
            LCD_handle_text(screen, 2, 0, signal_2);
            break;
        case 3:
            LCD_handle_text(screen, 0, 0, signal_1);
            LCD_handle_text(screen, 2, 0, signal_2);
            LCD_handle_text(screen, 4, 0, signal_3);
            break;
        case 4:
            LCD_handle_text(screen, 0, 0, signal_1);
            LCD_handle_text(screen, 2, 0, signal_2);
            LCD_handle_text(screen, 4, 0, signal_3);
            LCD_handle_text(screen, 6, 0, signal_4);
            break;
        case 5:
            LCD_handle_text(screen, 0, 0, signal_1);
            LCD_handle_text(screen, 2, 0, signal_2);
            LCD_handle_text(screen, 4, 0, signal_3);
            LCD_handle_text(screen, 6, 0, signal_4);
            LCD_handle_text(screen, 8, 0, signal_5);
            break;
        default:
            LCD_Graphic_send_text(screen, "         ", 0, 0);
            break;
    }
...
void Date_DMY(uint8_t *screen, uint8_t screen_num, uint8_t day, uint8_t date, uint8_t month, uint16_t year) {      // date: 1 = Sunday, 2 = Monday, ... , 7 = Saturday
    ...
    switch (day) {
        case 0:
            strcpy((char *)str_day, "Sun");
            break;
        case 1:
            strcpy((char *)str_day, "Mon");
            break;
        case 2:
            strcpy((char *)str_day, "Tue");
            break;
        case 3:
            strcpy((char *)str_day, "Wed");
            break;
        case 4:
            strcpy((char *)str_day, "Thu");
            break;
        case 5:
            strcpy((char *)str_day, "Fri");
            break;
        case 6:
            strcpy((char *)str_day, "Sat");
            break;
        default:
            strcpy((char *)str_day, "Day");
            break;
    }
...
void app_main(void) {
    ...
    LCD_I2C_init(&bus_handle, &LCD_handle);
    ...
    Date_DMY(Screen, SCREEN_NUM_1, 1, 30, 12, 2024);
    HMS(Screen, SCREEN_NUM_1, 23, 30, 00);
...
/*  END Group  */
    ...
    // LCD_write(0x80 + 0, 0);
    // LCD_send_text("DV_1");
    // sprintf(mess, "%3.2fV %1dA", volt_1, I1);
    // LCD_write(0x80 + 3, 0);
    // LCD_send_text(mess);

    // LCD_write(0x90 + 0, 0);
    // LCD_send_text("DV_2");
    // sprintf(mess, "%3.2fV %1dA", volt_2, I2);
    // LCD_write(0x90 + 3, 0);
    // LCD_send_text(mess);
    ...
    // LCD_write(0x80 + 8, 0);
    // LCD_send_text("DV_3");
    // sprintf(mess, "%3.2fV %1dA", volt_3, I3);
    // LCD_write(0x80 + 11, 0);
    // LCD_send_text(mess);

    // LCD_write(0x90 + 8, 0);
    // LCD_send_text("DV_4");
    // sprintf(mess, "%3.2fV %1dA", volt_4, I4);
    // LCD_write(0x90 + 11, 0);
    // LCD_send_text(mess);
